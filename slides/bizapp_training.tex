...
\part{{\bfseries\sffamily WIP} [講義] Ruby on Railsの開発}
\label{sec-7}
\section{Memo}
\label{sec-7-1}
\begin{itemize}
\item Gemfileのtherubyracerをアンコメントする
\end{itemize}
\section{Ruby on Railsの全体像}
\label{sec-7-2}
\section{Scaffoldの作成と動作}
\label{sec-7-3}
\begin{frame}[label=sec-7-3-1]{Herokuへのdeploy}
\end{frame}
\begin{frame}[label=sec-7-3-2]{ScaffoldによるModelの拡張}
\end{frame}
\section{RSpecによるテスト}
\label{sec-7-4}
\begin{frame}[label=sec-7-4-1]{RSpecの実行}
\end{frame}
\section{Travis CI}
\label{sec-7-5}
\begin{frame}[label=sec-7-5-1]{Travis CIのWeb管理画面}
\end{frame}
\begin{frame}[label=sec-7-5-2]{GitHubとTravis CI連携}
\end{frame}
\begin{frame}[label=sec-7-5-3]{Travis経由でのHerokuへのdeploy}
\end{frame}
