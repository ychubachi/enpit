...
\end{block}
\end{frame}
\part{{\bfseries\sffamily WIP} 第6章 Ruby on Railsアプリの開発}
\label{sec-6}
...
\end{block}
\end{frame}
\begin{frame}[fragile,label=sec-6-1-2]{\texttt{rails\_enpit} リポジトリを作成する}
 \begin{block}{内容}
\begin{itemize}
...
\end{block}
\end{frame}
\begin{frame}[fragile,label=sec-6-1-3]{Gemfileの変更}
 \begin{block}{変更する内容}
\begin{itemize}
...
\end{block}
\end{frame}
\begin{frame}[fragile,label=sec-6-1-4]{Bundle installの実行}
 \begin{block}{\texttt{bundle install}}
\begin{itemize}
\item Gemfileを読み込み，必要なgemをインストールする
\item therubyracerと，それが依存しているgemでまだインストールしていないものをインストール
...
\end{block}
\end{frame}
\begin{frame}[fragile,label=sec-6-1-5]{Rails serverの起動}
...
\end{block}
\end{frame}
\section{Scaffoldの作成と実行}
\label{sec-6-2}
\begin{frame}[fragile,label=sec-6-2-1]{Scaffold}
...
\end{block}
\end{frame}
\begin{frame}[fragile,label=sec-6-2-2]{DBのmigrate}
...
\end{block}
\end{frame}
\begin{frame}[label=sec-6-2-3]{動作確認}
\begin{block}{動作確認の方法}
\begin{itemize}
...
\section{RoRアプリのテストについて}
\label{sec-6-3}
\begin{frame}[fragile,label=sec-6-3-1]{テストについて}
 \begin{block}{ガイド}
\begin{itemize}
...
\end{block}
\end{frame}
\section{Herokuで動作するアプリ}
\label{sec-6-4}
\begin{frame}[label=sec-6-4-1]{Herokuで動かす場合の注意}
\begin{block}{DBについて}
\begin{itemize}
\item DatabeseはPostgreSQLを使用する
\begin{itemize}
\item sqliteは使えない
\item mysqlを利用することも可能
\end{itemize}
\end{itemize}
\end{block}
...
\begin{frame}[label=sec-6-4-2]{Scaffoldの作成}
\begin{block}{Herokuで動作する}
\end{block}
\end{frame}
\begin{frame}[fragile,label=sec-6-4-3]{DBの作成}
 \begin{minted}[]{bash}
createdb heroku_enpit_development
createdb heroku_enpit_test
\end{minted}
\end{frame}
\section{Travis CIとの連携}
\label{sec-6-5}
\begin{frame}[label=sec-6-5-1]{Travis CIのWeb管理画面}
\end{frame}
\begin{frame}[label=sec-6-5-2]{GitHubとTravis CI連携}
\end{frame}
\begin{frame}[label=sec-6-5-3]{Travis経由でのHerokuへのdeploy}
\end{frame}
\part{第7章 Web APIを活用したサービス構築}
\label{sec-7}
\section{ご利用ガイド}
\label{sec-7-1}
\begin{itemize}
...
\section{バージョン管理の概念}
\label{sec-13-1}
\begin{frame}[label=sec-13-1-1]{シナリオ}
...
\begin{frame}[label=sec-13-1-2]{バージョン管理の基礎知識}
\begin{block}{diff}
\end{block}
\begin{block}{patch}
\end{block}
...
\end{block}
\end{frame}
\begin{frame}[label=sec-13-1-3]{演習課題}
\begin{block}{演習課題}
\begin{itemize}
...
\begin{frame}[fragile,label=sec-13-1-4]{GitHubにpush}
 \begin{block}{コマンド}
\begin{minted}[]{bash}
