...
*** コンフリクトの解消
**** コンフリクトが発生したら
     - origin に新しいコミットがマージされたら，
       コンフリクトが発生してマージできなくなることがある
**** コマンド（未確認）
#+begin_src bash
git fetch
...
*** (1) ファイルを追加する
**** 演習課題
    1. =ychubich/our_enpit= をgitコマンドでcloneし，forkする
...
**** 終わったブループは…
     - リポジトリのcontributerにメンバーを登録する方法も試す
